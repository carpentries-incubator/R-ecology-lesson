...
Some dates failed to parse, and they now show up as `NA` in the `date` column of `surveys_new`. Figure out what those dates were and why they might have been wrong.
...
```{r}
read_csv("episodes/data/raw/surveys_new.csv") %>% 
...
```{r read-species-new}
species_new <- read_delim("episodes/data/raw/species_new.txt", delim = " ", quote = '"')
...
It looks like our data are in a bit of a strange format. We have a column for each plot, and then a single row of data containing the plot type. If you look at our old `surveys` data, we had a single row for `plot_id` and a single row for `plot_type`. However, our data are in a **wide** format. In the last episode, we took some **long** data and used `pivot_wider()` to reshape it into a wide format. There is a similar function to move from wide to long format, called `pivot_longer()`.

We pipe the `plots_new` data into `pivot_longer()`, and then add 3 other argumens. `cols` takes the names of the columns that we want to pivot; in our case, we want all the columns, so we can use the function `everything()` to use all the columns. `names_to` is the name of the new column that will contain the old column names, and `values_to` is the name of the new column that will contain the values in the old columns.
...
```{r read-old-surveys}
surveys <- read_csv("episodes/data/cleaned/surveys_complete_77_89.csv")
```
