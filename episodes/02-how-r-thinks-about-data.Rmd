...
The `surveys` data is stored in R as a **data.frame**, which is the most common way that R represents tabular data (data that can be stored in a table format, like a spreadsheet). We can check what `surveys` is by using the `class()` function:
...
Let's get back to investigating our `surveys` data.frame. We can get some useful summaries of each variable using the `summary()` function:
...
You might have noticed that our last result looked different from when we printed out the `surveys` data.frame itself. That's because it is not a data.frame, it is a **vector**. A vector is a 1-dimensional series of values, in this case a vector of numbers representing years.
