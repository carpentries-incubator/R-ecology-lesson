...
- I think it might be worth addressing tab completion here somewhere
- also worth adding comments at this point
...
and removing the legend. You should also increase the font
size for the plot title and make it bold. *Hint*: "bold"
is considered a "face" of a font.

:::::::: solution

```{r theme-challenge-answer, answer=TRUE, purl=FALSE}
ggplot(data = surveys, mapping = aes(x = plot_type, y = hindfoot_length)) +
  geom_point(aes(color = plot_type), position = position_jitter(), alpha = 0.2) +
  geom_boxplot(fill = NA) +
  theme_bw() +
...
```{r theme-challenge, echo=FALSE, eval=FALSE, purl=TRUE}
## Modify the previous plot by adding a descriptive subtitle
## and removing the legend. You should also increase the font
## size for the plot title and make it bold. *Hint*: "bold"
## is considered a "face" of a font.
...
Try making a plot of your own! You can further modify a
previous plot or make a new one altogether. Try using
`str(surveys)` to explore variables you might be able to
use.

Here are some examples:

- make a histogram of one of the numeric variables
- use a different color `scale_`
- change the size of points or thickness of lines in a `geom`
