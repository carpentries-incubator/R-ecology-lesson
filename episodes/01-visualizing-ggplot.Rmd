...
:::::::::::::::::::::::::::::::::::::::::::: instructor
The `ratdat` package is in the process of being published on CRAN. When this happens, you will be able to install it with `install.packages("ratdat")` and then use the `complete_old` data.frame, which matches the one created below. So the use of `dplyr` to create `surveys` will be removed soon.
::::::::::::::::::::::::::::::::::::::::::::
...
```{r no-closing-bracket, eval=F, warning=F}
ggplot(data = surveys, mapping = aes(x = weight, y = hindfoot_length)) +
...
```{r no-closing-quote, eval=F, warning=F}
ggplot(data = surveys, mapping = aes(x = weight, y = hindfoot_length)) +
  geom_point(color = "blue, alpha = 0.2)
```
...
```{r extra-bracket, eval=F, warning=F}
ggplot(data = surveys, mapping = aes(x = weight, y = hindfoot_length)) +
  geom_point(color = "blue", alpha = 0.2))
```
...
```{r modify-points-challenge-answer, warning=F}
ggplot(data = surveys, 
...
```{r modify-color-challenge-answer, warning=F}
ggplot(data = surveys, 
...
```{r violin-challenge-answer, warning=F}
ggplot(data = surveys, 
...
```{r violin-challenge-answer-2, warning=F}
ggplot(data = surveys, 
       mapping = aes(x = plot_type, 
                     y = hindfoot_length,
                     color = plot_type)) +
...
```{r customizing-challenge-answer, warning=F}
ggplot(data = surveys, mapping = aes(x = plot_type, y = hindfoot_length)) +
  geom_point(aes(color = plot_type), position = position_jitter(), alpha = 0.2) +
  geom_boxplot(fill = NA) +
  theme_bw() +
...
```{r patchwork-annotation, warning=F}
